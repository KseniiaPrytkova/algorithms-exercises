...
class Node {
	constructor(string) {
		// node'll be build itself with all it's children
		// if u pass 'boston', I'll make the value 'b' and I'll create a new node in my children of 'oston'
		this.children = [];
		// am I on valid word right now
		// flag marking end of the word
		this.terminus = false;
		// 1st letter
		this.value = string[0];
		if (string.length > 1) {
			// recursivelly create new nodes of string substring
	...
	add(string) {
		...
		for (let i = 0; i < this.children.length; i++) {
			// looking here for where we're gonna put our new string whenever we called add
			const child = this.children[i];

			if (child.value === value) {
				if (next) {
					// or we have more to add
					child.add(next);
				} else {
					// or we reached the end - now we have a complete string
					child.terminus = true;
				}
				return;
			}
	...
	_complete(search, built, suggestions) {
		// we only will suggest top 3 at a time
